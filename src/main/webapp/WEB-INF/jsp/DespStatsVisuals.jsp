...
<div class="row">
	...
	style="color: black; background-color:#DCDCDC;">
		...
		<form method="GET" class="px-2 py-2" action="/query">
				...
				<label>Views</label> <select name="views" class="form-control" id="viewSelected">
	...
	<div class="container col-2">
		...
		<h6>Descriptive Statistics</h6>
			...
			<div class="btn-group dropright span9 btn-block no-padding">
				...
				<div class="dropdown-menu">
					<button class="btn btn-secondary right-button" type="button"
						onclick="calphenomean()">View mean values</button>
					<button class="btn btn-secondary right-button" type="button"
						onclick="calphenmedian()">View median values</button>
					<button class="btn btn-secondary right-button" type="button"
						onclick="calphenstdev()">View standard deviation values</button>
				</div>
			...
			<div class="btn-group dropright span9 btn-block no-padding">
				<button type="button"
					class="btn btn-success btn-block dropdown-toggle"
					data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">Visualizations</button>
				<div class="dropdown-menu">
					<button class="btn btn-secondary right-button" type="button"
						onclick="singlePlantMultipleViews()">Single Plant, Multiple Views</button>
					<button class="btn btn-secondary right-button" type="button"
						onclick="singleViewMultiplePlants()">Multiple Plants, Single View</button>	
				</div>
			</div>
	...
	<div class="container col-8 ScrollStyle" id="results-div">
		...
		<script type="text/javascript">
			...
			function getDateVsChaChartSingViewMulPlants(dataArray,plantIds,viewVal) {
				...
				for(var i=0; i< plantIds.length;i++){
				for (var j = 0; j < dataArray.length; j++) {
					if ((dataArray[j].view == viewVal) && plantIds[i] ==dataArray[j].image.plant.id) {
						r += dataArray[j].date;
						r += "," + dataArray[j].convexHullArea;
					}
					r += "\n";
			...
			function singleViewMultiplePlants(event) {
				...
				$.get(
						"http://localhost:8080/PhenotypeAnalysis/web/phenotypesbyspplnts/"
							...
							data1 = getDateVsChaChartSingViewMulPlants(opts,plantIds,viewVal);
